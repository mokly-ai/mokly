...
- [x] Adopt direct and alias URL variants/fragments on the first hydrated
      render and keep query-only route changes visible in the catalogue rail.
...
- [ ] Preserve cancellation through pending usage updates and geometry reads:
...
- [ ] Adopt a routed page's public catalogue, private workspace and source
...
- [ ] Preserve supplied Changes evidence in the public projection when live
      Changes updates are disabled, while continuing to omit unknown live
      states from static capture.
- [ ] Load matching route evidence during static in-shell navigation so
...
- [ ] Profile and correct the full design-catalogue export timeout, retaining
...
- [ ] Rewrite every unit test and test helper that imports a retired module
      ...
      milestone.
- [ ] Retain the complete native shell-link eligibility matrix and denied
...
- [ ] Rerun the packed-consumer smoke and the published-package layout checks
      for both tarballs.
- [ ] Run `cargo xtask check` on the integrated delivery and completed host
...
- [ ] Restore embedded root sizing and containment, preview-only host overlay
      bounds and independent inspector resizing under the shared shell layout.
- [ ] Preserve public selection and comparison error reporting, Escape pick
      cancellation, viewport/scheme replacement cancellation, and complete
      frame cleanup even when a host callback throws.
- [ ] Keep standalone and embedded search lossless through the shared action
      boundary, with sequential input tests for spaces and typed tag terms.
- [ ] Preserve exact saved-variant and repeated-flow inspection scope, atomic
...
- [ ] Preserve replacement inspection requests and current picks when obsolete
...
- [ ] Exercise retained-evidence restoration through the production host bridge,
...
- [ ] Keep a valid existing highlight when a replacement request fails input
      validation, while clearing a current presentation that fails after its
      activation has begun.
- [ ] Preserve active inspection through slot, marker and callback updates that
...
- [ ] Mark only the first flow step as the logical fragment target, with
      separate assertions for rendered iframe attributes and bootstrap data.
- [ ] Keep nested-instance disclosure on the Components panel while explicit
