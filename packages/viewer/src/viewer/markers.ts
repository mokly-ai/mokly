...
import { matchesInstanceFrame, hasInstance } from "./frame_views.js";
import type { GeometryRefresh, GeometrySnapshot } from "./geometry_refresh.js";
import { markerRect, markerStatus } from "./marker_geometry.js";
import type { CoordinateRect } from "./marker_geometry.js";
import type { MarkerPlacement, MarkerStore } from "./marker_store.js";
import type {
  InstanceRef,
  MarkerState,
  ViewerEvents,
...
/** Match host markers to cached authenticated geometry without exposing boxes. */
export class FrameMarkers {
  ...
  update(markers: readonly ViewerMarker[]): void {
    ...
    const invalid = ids.size !== targets.length;
    if (
      this.initialized &&
  ...
  demand(): readonly Session[] {
    ...
    return this.sessions().filter(({ frame }) =>
      this.targets.some(
        ({ instance }) =>
  ...
  private evaluate(): void {
    ...
    for (const target of this.targets) {
      const session = this.sessions().find(({ frame }) =>
        matchesInstanceFrame(frame, target.instance),
      );
      const usage = session?.frame.view?.usage;
      let usageReady: boolean;
      try {
        usageReady = usage?.status === "ready" && !frameUsage(usage).error;
      } catch {
        usageReady = false;
      }
      const present = usageReady && hasInstance(usage, target.instance.key);
      const snapshot = session
        ? this.geometry.snapshot(session)
        : ({ pending: false, cycle: 0 } satisfies GeometrySnapshot);
      const boundary =
      ...
      const status = markerStatus({
        current,
        matched: Boolean(session?.mounted && session.frame.element.isConnected),
        usageReady,
        present,
  ...
  private place(
    ...
    const frame = session.frame.element;
