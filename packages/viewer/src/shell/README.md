...
hidden controls out of the hydrated document. The CLI supplies its private live capabilities through typed server
context. Standalone full-document composition lives in `src/standalone`: its
bootstrap contains only the validated public catalogue and shell delivery
state, and `src/browser.tsx` hydrates that exact server tree.
