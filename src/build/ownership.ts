...
export function ownedGeneratedRoutes(config: ResolvedConfig): string[] {
  return walkFiles(config.generatedDir)
    .filter((candidate) => isOwned(candidate, config))
    .map((candidate) =>
      toPosixPath(path.relative(config.generatedDir, candidate)),
    )
    .sort();
}
  ...
  return walkFiles(config.generatedDir)
    .flatMap((candidate) => {
      const relative = toPosixPath(
        path.relative(config.generatedDir, candidate),
      );
...
export function generatedOwnershipDenial(
  ...
  try {
    if (!isInside(config.generatedDir, candidate))
      return "is outside generatedDir";
