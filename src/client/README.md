...
Each route or live-evidence page contributes a strictly validated route-scoped
public bootstrap and a complete private workspace. They are adopted as one
revision by replacing the prior scoped catalogue, never merging usage across
visited routes. Until adoption, the shell reports Usage as loading; a current
read failure or rejection reports failed and supports retry without remounting
the preview. Authored content changes retain the reload lifecycle.
