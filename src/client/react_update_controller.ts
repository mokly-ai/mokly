/** Versioned live-update coordination and one-shot React shell recovery. */
...
/** Event stream subset used by the React host. */
...
const recoveryKey = "mokly:live-update-recovery";

/** Coordinate latest-wins refreshes and one-shot reload recovery. */
export class ReactUpdateController {
  #lastVersion: number;
  ...
  /** Close the stream and cancel the current evidence refresh. */
  close(): void {
    this.#closed = true;
    this.#pending?.abort();
    this.stream.close();
  ...
  /** Consume recovery exactly once; later reads see no stale state. */
  ...
  private reload(version: number): void {
    ...
    this.storage.setItem(recoveryKey, JSON.stringify(recovery));
