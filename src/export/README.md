...
independent `BaselineReader`, compiles in memory without writing the catalogue,
...
Current-only exports skip historical preparation entirely, retain compiled
generated bytes, and still reject authored source or resource edits during capture.
Every export includes the standalone hydration bundle and matching document
...
With comparisons enabled, every export awaits `prepareReviewRepository` before
