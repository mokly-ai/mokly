...
published together. Local Serve and export share the standalone presentation;
embedded hosts render the same shell with host-owned appearance.
...
in the same screen, with mobile/desktop controls and the catalogue-wide
Appearance setting, secondary impact evidence, and a refresh option. Loading
and failure states keep the catalogue available and offer a retry. Navigation
and reload return to Current.
...
All 85 design screens reuse the 16 registered components in
