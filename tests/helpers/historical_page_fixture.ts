...
export const mockups = [
  ...
  if (options.symlink) await fs.symlink("document.css", artifact);
    ...
    await fs.writeFile(
      artifact,
      `<!-- Generated by mokly from ${sourcePath}. Do not edit. -->\n${(options.document ?? pageDocument).replace('href="../document.css"', 'href="document.css"')}\n`,
    );
  fixture.git("add", "-A");
  ...
  return {
    ...
    currentPath,
    manifest,
    async headReader(document = currentDocument) {
      const compilation = await compileCatalogue(fixture.config);
      return new CompiledReviewAssetReader(
        fixture.config,
        new Map([...compilation.outputs, ["handbook.html", document]]),
      );
    },
  };
}
