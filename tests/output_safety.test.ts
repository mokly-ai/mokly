...
test("generated ownership belongs only to configured source roots", async (context) => {
  ...
  await fs.promises.mkdir(config.generatedDir);
  const foreign = path.join(config.generatedDir, "foreign.html");
  const deleted = path.join(config.generatedDir, "deleted.html");
