...
test("Browse treats the generated notice as text, but rejects broken navigation markers", async (context) => {
  const fixture = await createFixture();
  context.after(() => removeFixture(fixture));
  const compilation = await compileCatalogue(await loadConfig(fixture.root));
  const catalogue = createCatalogue(compilation.manifest);
  ...
  for (const notice of [
    "",
    "<!-- Generated by mokly from entries/other.ts. Do not edit. -->\n",
  ])
    assert.doesNotMatch(
      adaptBrowseDocument(
        original.replace(GENERATED_MARKER, notice),
        route,
        catalogue,
      ),
      /Generated by/,
...
test("Browse accepts and removes CRLF generated notices", async (context) => {
  const fixture = await createFixture();
  context.after(() => removeFixture(fixture));
  const compilation = await compileCatalogue(await loadConfig(fixture.root));
  const route = "screens/home.mobile.html";
  ...
  assert.doesNotMatch(adapted, /Generated by Mokly/);
});
