...
design catalogue in the basic example, whose entry ids start with `design-`;
...
generated under `examples/basic/generated/screens/` at each entry's derived
route, `screens/<id>.html`. This Browse/Changes table and the
[component design inventory](./mokly-component-design.md#owning-catalogue)
together define the complete set of Mokly shell design-screen ids, matching the
manifest generated by `npm run example:build`; the Folder column is the entry's
`navPath` below the `Design › Mokly design` root. Update the owning inventory
when design entries change and verify exact id agreement with that local
...
- **Screen head** — surface band with the breadcrumb trail (11.5px, `›`
  separators; ancestor crumbs that resolve to a viewable entry are links) and
...
- **Details inspector** — the shared icon footer opens the chosen tab in place;
  ...
  rows (Source, Schemes, Changed views, Tags, Related docs,
