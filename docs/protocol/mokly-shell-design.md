...
- **Top bar** — 48px, surface background, hairline bottom border: the Mokly
  ...
  and `M14 20h7`, two-unit round-capped strokes) in `--chrome-surface`. The
  wordmark is 20px `--serif` at regular weight in the chrome ink, with
  `-0.01em` tracking, and never wraps. There is no mode
