...
- `design/browse/pages/previous-version/` and
  ...
  pages (loading, unavailable with retry, and long-document scrolling) planned
  ...
  The depicted branch removes four documents and four screens, so each child is
  ...
  repeats the request, so it returns to the loaded preview. Served and exported
