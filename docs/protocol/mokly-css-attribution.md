...
without decoding it. Preserve this delivered-source list across the retained
runtime and background classification; a direct classification without it
rederives the inventory without evaluating consumer JavaScript for relevant
CSS-input **or asset-only** changes. For committed output, obtain the accepted
route index and delivered-source inventory in the same inventory-only load.
