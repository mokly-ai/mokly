...
Approved target for [Generated Output Simplification](../../plans/generated-output-simplification.md).
The implementation is staged: this contract describes the completed behavior,
not the current behavior during intermediate milestones. See
...
expected files missing; an unexpected path, including an unexpected symlink,
is extra. If any mismatch exists, fail `build-invalid` with this exact text:
...
Stage the **entire** `.generated/` tree in a sibling temporary directory;
validate it before installation, move the old tree aside, install the new
tree, and restore the old tree if installation fails. Clean temporary and
backup trees only after a successful install or rollback. There is no
per-file overwrite refusal, orphan or unclaimed-file scan, or ownership
header parser. All previous files inside `.generated/` are disposable;
nothing outside it is replaced by a build. Generated HTML starts with the
single plain-text line `<!-- Generated by Mokly. Do not edit. -->`; nothing
