...
`mokly build` writes deterministic output under `<mockupsDir>/.generated/`:
...
previous dark documents extra files when output is tracked: `check` reports
them and `build` replaces the entire generated tree.

Manifest source and output paths are repository-relative; routes are relative
to `.generated/`. The manifest includes every entry, fragment, source input,
...
Generated documents start with `<!-- Generated by Mokly. Do not edit. -->`.
Nothing parses that line. Build replaces only the entire `.generated/` tree,
not files alongside authored assets. The manifest carries a sorted referenced
asset closure and Git blob-hash inventory (excluding itself), with the exact
schema and hash algorithm in [generated output](./mokly-generated-output.md).

All catalogues emit manifest v6, retaining the
[component manifest](./mokly-component-manifest.md) fields, including
pages, source inventory, saved component variants and per-view invocation/ownership
records. Historical readers also accept v5, v3, both disjoint v4 formats, and opt-in v2.
